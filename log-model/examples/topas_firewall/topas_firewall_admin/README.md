...
事件事实：防火墙管理员日志——用户 superman 通过串口（Serial）本地登录防火墙（TopsecOS，192.0.2.85），result=success，login success。

- 主体：管理员 → `roles.source.user{name=superman}` + `source.endpoint(192.0.2.85)`
...
- 原始样本：`s4-doris/models/wpl/topas_firewall/sample.dat` 第 1 个非空行。
