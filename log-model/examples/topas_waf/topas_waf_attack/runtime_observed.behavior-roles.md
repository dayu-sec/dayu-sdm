...
| 角色 | 判定 | 证据 |
|---|---|---|
| subject | `endpoint` `192.0.2.188:64544` | `client_ip` / `sport` / `real_ip` |
| object | `endpoint` `192.0.2.187:80` | `server_ip` / `dport`。旧样例同时挂了 `host.name=test`，违反「只能选一」。HTTP Host 用 raw `host=192.0.2.187`；`server=test` 进 `source_private.server` |
