...
| `computer_name` | "DESKTOP-EX07" | `roles.target.host.name` | `roles_obj.target.host.name` | "DESKTOP-EX07" | 按当前 expected 事件结构映射 |
...
| `wpl.computer_name` | `source_host` | "DESKTOP-EX07" | 由 WPL 字段 computer_name 投影或转换后赋值 |
| `wpl.computer_name` | `target_host` | "DESKTOP-EX07" | 由 WPL 字段 computer_name 投影或转换后赋值 |
...
| `roles.target.host.name` | `projection` | `computer_name` | "DESKTOP-EX07" | `—` | confirmed；按当前 expected 事件结构映射 |
