...
| `mac` | "00-50-56-81-F3-29" | `roles.target.host.mac` | `roles_obj.target.host.mac` | "00:50:56:81:f3:29" | 按当前 expected 事件结构映射 |
...
| `risky_source` | {"disposal_advice": "对终端进行断网操作，以避免影响其他终端", "ip": "198.51.100.23", "disposal_advice_i18n": {"zh_TW": "對終端進行斷網操作，以避免影響其他終端", "en_US": "Disconnect the terminal to prevent it from affecting other terminals", "zh_CN": "对终端进行断网操作，以避免影响其他终端"}, "mac": "00-50-56-81-F3-30"} | 当前事件未采用该字段；空值、哨兵、机械重复或非检索上下文不落库，原值保留在 raw_msg |
...
| `wpl.risky_source.mac` | `extensions.source_private.risky_source.mac` | "00-50-56-81-F3-30" | 由 WPL 字段 risky_source.mac 投影或转换后赋值 |
...
| `roles.target.host.mac` | `projection` | `mac` | "00:50:56:81:f3:29" | `—` | confirmed；按当前 expected 事件结构映射 |
