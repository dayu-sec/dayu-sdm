...
> 样例：`remote_process_start.expected-sdm-event.json`；WPL 字段 33 个，映射 21，不落库 12，排除 0；另有平台默认、派生及结构字段 44 项。
...
|---|---|---|---|---|---|
| `alert_id` | 2880734758977974811 | `source_original_event_id` | `source_original_event_id` | "2880734758977974811" | 按当前 expected 事件结构映射 |
...
| `rule_id` | "30.0.0001" | `source_finding.rule.signature_id` | `source_finding_obj.rule.signature_id` | "30.0.0001" | 按当前 expected 事件结构映射 |
...
| `tactic` | "Lateral Movement" | `source_finding.mitre.tactics[0]` | `source_finding_obj.mitre.tactics[0]` | "Lateral Movement" | 按当前 expected 事件结构映射 |
| `technique` | "Lateral Tool Transfer" | `source_finding.mitre.techniques[0]` | `source_finding_obj.mitre.techniques[0]` | "Lateral Tool Transfer" | 按当前 expected 事件结构映射 |
| `technique_id` | "T1570" | `source_finding.mitre.technique_id` | `source_finding_obj.mitre.technique_id` | "T1570" | 按当前 expected 事件结构映射 |
| `alert_name` | "被远程启动\"cmd.exe\"进程" | `source_finding.title` | `source_finding_obj.title` | "被远程启动\"cmd.exe\"进程" | 按当前 expected 事件结构映射 |
| `description_i18n/zh_CN` | "由于检测到PsExec的启动，所以产生了告警。PsExec是一款常见的横向渗透工具，常被利用于内网渗透" | `source_finding.description` | `source_finding_obj.description` | "由于检测到PsExec的启动，所以产生了告警。PsExec是一款常见的横向渗透工具，常被利用于内网渗透" | 按当前 expected 事件结构映射 |
...
| `description` | "由于检测到PsExec的启动，所以产生了告警。PsExec是一款常见的横向渗透工具，常被利用于内网渗透" | 当前事件未采用该字段；空值、哨兵、机械重复或非检索上下文不落库，原值保留在 raw_msg |
| `report_ip` | "198.51.100.114" | 疑似 Agent 上报链路地址；与终端 IP 不一致时不作为事件 source IP，原值保留在 raw_msg |
| `tactic_id` | "TA0008" | 当前事件未采用该字段；空值、哨兵、机械重复或非检索上下文不落库，原值保留在 raw_msg |
| `type` | "edr_alert" | 仅用于 WPL 路由，稳定日志类型已由 log_type 表达；原值保留在 raw_msg |
| `alert_name/zh_TW` | "已遠端啟動\"cmd.exe\"進程" | 当前事件未采用该字段；空值、哨兵、机械重复或非检索上下文不落库，原值保留在 raw_msg |
| `alert_name/en_US` | "The \"cmd.exe\"  process was started remotely" | 当前事件未采用该字段；空值、哨兵、机械重复或非检索上下文不落库，原值保留在 raw_msg |
| `alert_name/zh_CN` | "被远程启动\"cmd.exe\"进程" | 当前事件未采用该字段；空值、哨兵、机械重复或非检索上下文不落库，原值保留在 raw_msg |
| `description_i18n/en_US` | "An alarm was generated because the startup of the PsExec was detected. PsExec is a common horizontal penetration tool, which is often used in intranet penetration." | 当前事件未采用该字段；空值、哨兵、机械重复或非检索上下文不落库，原值保留在 raw_msg |
| `description_i18n/zh_TW` | "由於檢測到PsExec啟動，因此生成警報。PsExec是一種常用的水準滲透工具，常用於企業內部網的滲透。" | 当前事件未采用该字段；空值、哨兵、机械重复或非检索上下文不落库，原值保留在 raw_msg |
| `risky_source` | {"ip": "198.51.100.23", "disposal_advice": "对终端进行断网操作，以避免影响其他终端", "disposal_advice_i18n": {"en_US": "Disconnect the terminal to prevent it from affecting other terminals", "zh_CN": "对终端进行断网操作，以避免影响其他终端", "zh_TW": "對終端進行斷網操作，以避免影響其他終端"}} | 当前事件未采用该字段；空值、哨兵、机械重复或非检索上下文不落库，原值保留在 raw_msg |
...
| `derived.sha256('|'+mapping_id+'|'+source_original_event_id)` | `event_id` | "evt-aebd208c6f4421e669dcb3223b4fa87f11f9069bc589b04b0050e0e777c78dec" | 按固定格式 sha256('|'+mapping_id+'|'+source_original_event_id) 生成；本样例 source_original_event_id 来自 WPL alert_id，tenant_id 为空不参与该公式 |
| `platform_context.log_id` | `log_id` | "log-tianqing-edr-alert-remote_process_start" | 由平台上下文提供 |
| `raw_log_input` | `raw_msg` | "{\"type\":\"edr_alert\",\"alert_id\":2880734758977974811,\"alert_name\":{\"zh_TW\":\"已遠端啟動\\\"cmd.exe\\\"進程\",\"en_US\":\"The \\\"cmd.exe\\\"  process was started remotely\",\"zh_CN\":\"被远程启动\\\"cmd.exe\\\"进程\"},\"gid\":\"4fdcff0b7800b21b\",\"group_name\":\"未分组终端\",\"mid\":\"81a15464af48af9f7fc8652fd820f9a75724c678e2920d7d5e7317fc57a2e900\",\"mac\":\"00-00-5E-00-53-FD\",\"computer_name\":\"EDRAUTOTEST-1717\",\"custom_group_paths\":\",2877377393142575643,4fdcff0b7800b21b\",\"report_ip\":\"198.51.100.114\",\"ip\":\"198.51.100.114\",\"client_id\":\"7036994-ffdc79ca939f96375c1605a3ba6d71ef\",\"asset_oid\":2715651939760080161,\"asset_id\":2838024086184002130,\"client_login_user\":\"\",\"severity\":4,\"category_id\":801,\"technique_id\":\"T1570\",\"tactic_id\":\"TA0008\",\"description\":\"由于检测到PsExec的启动，所以产生了告警。PsExec是一款常见的横向渗透工具，常被利用于内网渗透\",\"description_i18n\":{\"en_US\":\"An alarm was generated because the startup of the PsExec was detected. PsExec is a common horizontal penetration tool, which is often used in intranet penetration.\",\"zh_CN\":\"由于检测到PsExec的启动，所以产生了告警。PsExec是一款常见的横向渗透工具，常被利用于内网渗透\",\"zh_TW\":\"由於檢測到PsExec啟動，因此生成警報。PsExec是一種常用的水準滲透工具，常用於企業內部網的滲透。\"},\"technique\":\"Lateral Tool Transfer\",\"tactic\":\"Lateral Movement\",\"create_time\":1717051732000,\"status\":0,\"rule_id\":\"30.0.0001\",\"process_details\":[{\"process_name\":\"explorer.exe\",\"process_id\":\"5526\",\"process_md5\":\"7a413ddd10e81adb6bb5d5e38f399d08\",\"process_path\":\"C:\\\\Windows\\\\explorer.exe\",\"process_sign\":\"Microsoft Windows\",\"process_command_line\":\"C:\\\\Windows\\\\Explorer.EXE\",\"process_create_time\":1717051719680,\"process_terminate_time\":0,\"process_guid\":\"4012f6f0f7cf4afe8838042a18080dec\",\"process_parent_guid\":\"e583b89c51e9246dbecd2abb7a784cc0\",\"child_process_details\":[{\"process_name\":\"cmd.exe\",\"process_id\":\"6585\",\"process_md5\":\"8a2122e8162dbef04694b9c3e0b6cdee\",\"process_path\":\"C:\\\\Windows\\\\system32\\\\cmd.exe\",\"process_sign\":\"Microsoft Windows\",\"process_command_line\":\"C:\\\\Windows\\\\system32\\\\cmd.exe\",\"process_create_time\":1717051719680,\"process_terminate_time\":0,\"process_guid\":\"c84108f5d67bbd5e0aa17618d3b593d3\",\"process_parent_guid\":\"4012f6f0f7cf4afe8838042a18080dec\",\"child_process_details\":[{\"process_name\":\"curl.exe\",\"process_id\":\"5041\",\"process_md5\":\"eac53ddafb5cc9e780a7cc086ce7b2b1\",\"process_path\":\"C:\\\\Windows\\\\System32\\\\curl.exe\",\"process_sign\":\"HaoShuQing\",\"process_command_line\":\"C:\\\\Windows\\\\System32\\\\curl.exe\",\"process_create_time\":1717051719680,\"process_terminate_time\":0,\"process_guid\":\"63fe679e70ab3c96e517da0be9f271d3\",\"process_parent_guid\":\"c84108f5d67bbd5e0aa17618d3b593d3\",\"is_alert_trigger\":true}]}]}],\"ioc_alerts\":[],\"risky_source\":{\"ip\":\"198.51.100.23\",\"disposal_advice\":\"对终端进行断网操作，以避免影响其他终端\",\"disposal_advice_i18n\":{\"en_US\":\"Disconnect the terminal to prevent it from affecting other terminals\",\"zh_CN\":\"对终端进行断网操作，以避免影响其他终端\",\"zh_TW\":\"對終端進行斷網操作，以避免影響其他終端\"}}}" | 由原始日志接入层保存，不由 OML 拼装 |
...
| `wpl.alert_id` | `source_finding_original_id` | "2880734758977974811" | 由 WPL 字段 alert_id 投影或转换后赋值 |
| `wpl.alert_name` | `source_finding_title` | "被远程启动\"cmd.exe\"进程" | 由 WPL 字段 alert_name 投影或转换后赋值 |
...
| `wpl.rule_id` | `source_finding_signature_id` | "30.0.0001" | 由 WPL 字段 rule_id 投影或转换后赋值 |
...
| `wpl.alert_id` | `source_finding.original_id` | "2880734758977974811" | 由 WPL 字段 alert_id 投影或转换后赋值 |
...
| `source_original_event_id` | `projection` | `alert_id` | "2880734758977974811" | `—` | confirmed；按当前 expected 事件结构映射 |
...
| `source_finding.rule.signature_id` | `projection` | `rule_id` | "30.0.0001" | `—` | confirmed；按当前 expected 事件结构映射 |
| `source_finding.status` | `projection` | `status` | "0" | `—` | confirmed；按当前 expected 事件结构映射 |
| `source_finding.mitre.tactics[0]` | `projection` | `tactic` | "Lateral Movement" | `—` | confirmed；按当前 expected 事件结构映射 |
| `source_finding.mitre.techniques[0]` | `projection` | `technique` | "Lateral Tool Transfer" | `—` | confirmed；按当前 expected 事件结构映射 |
| `source_finding.mitre.technique_id` | `projection` | `technique_id` | "T1570" | `—` | confirmed；按当前 expected 事件结构映射 |
| `source_finding.title` | `projection` | `alert_name` | "被远程启动\"cmd.exe\"进程" | `—` | confirmed；按当前 expected 事件结构映射 |
| `source_finding.description` | `projection` | `description_i18n/zh_CN` | "由于检测到PsExec的启动，所以产生了告警。PsExec是一款常见的横向渗透工具，常被利用于内网渗透" | `—` | confirmed；按当前 expected 事件结构映射 |
