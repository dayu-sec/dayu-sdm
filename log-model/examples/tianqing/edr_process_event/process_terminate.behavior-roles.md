...
- `computer_name`：raw `DESKTOP-EX02` vs WPL `DESKTOP-EX01`，信封跟 WPL。
- `ip`/`client_ip=192.0.2.183` 进 `profiles.host.ip`；`report_ip`/`client_report_ip=192.0.2.190` 进 `source_private.report_ip`（不是同一地址，不能当别名吞掉）。
