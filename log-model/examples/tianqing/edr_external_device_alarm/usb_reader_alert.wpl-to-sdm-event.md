...
> 样例：`usb_reader_alert.expected-sdm-event.json`；WPL 字段 36 个。

## 一、WPL 字段映射

| WPL 字段 | WPL 值 | SDM 落位 | expected 位置 | 说明 |
|---|---|---|---|---|
| `guid` | "{4D36E967-E325-11CE-BFC1-08002BE10318}" | `source_original_event_id` | `source_original_event_id` | 去花括号 |
| `name` | "MULTIFLASHREADERUSBDEVICE" | `roles.related[].device.name` | `roles_obj.related[].device.name` | 外设名 |
| `vid` | "12738" | `extensions.source_private.vid` + `related.device.vendor_id` | `extensions_obj.source_private.vid` | 厂商 ID |
| `pid` | "2134" | `extensions.source_private.pid` + `related.device.product_id` | `extensions_obj.source_private.pid` | 产品 ID |
| `result` | 1 | `extensions.source_private.result` + `outcome=allowed` | `outcome` | PDF §3.6.5 放行 |
| `report_ip` | "198.51.100.63" | `roles.target.host.ip` | `roles_obj.target.host.ip` | 受管终端上报 IP |
| `mac` | "00-0C-29-14-90-32" | `roles.target.host.mac` | `roles_obj.target.host.mac` | 受管终端 MAC |
...
1. 06 枚举无外部设备告警类型，保持 generic_event；逻辑 fixture 的 `external_device_alarm` 不写入物理 `event_type`
...
| `roles.target.host.id(profiles.subject_ref)` | `context` | `platform_context.asset_id` | `platform_context.asset_id` | `—` | confirmed；受管终端是客体 |
| `event_type` | `constant` | `constant` | "generic_event" | `—` | confirmed；06 无此外设类型 |
| `event_category` | `constant` | `constant` | "alert" | `—` | confirmed；外设管控告警 |
| `event_domain` | `constant` | `constant` | "endpoint" | `—` | confirmed；见 README 决策 |
| `record_kind` | `constant` | `constant` | "finding" | `—` | confirmed；见 README 决策 |
| `outcome` | `constant` | `constant` | "allowed" | `—` | confirmed；PDF §3.6.5 result=1 放行 |
| `observer_vendor` | `constant` | `constant` | "qax" | `—` | confirmed；产品身份 |
| `observer_product` | `constant` | `constant` | "tianqing" | `—` | confirmed；产品身份 |
| `roles.related[].ref_id` | `derived` | `derived.entity_ref(device, raw_log.device_instance_path)` | `derived.entity_ref(device, raw_log.device_instance_path)` | `—` | confirmed；外设 related.device |
...
| `mapping_id` | `constant` | `constant.mapping_id` | "qax.tianqing.edr_external_device_alarm" | `—` | confirmed；expected 事件确认的日志类型级常量 |
| `data_src_product` | `constant` | `constant.data_src_product` | "tianqing" | `—` | confirmed；expected 事件确认的日志类型级常量 |
| `log_type` | `constant` | `constant.log_type` | "edr_external_device_alarm" | `—` | confirmed；expected 事件确认的日志类型级常量 |
| `data_src_category` | `constant` | `constant.data_src_category` | "endpoint_security" | `—` | confirmed；expected 事件确认的日志类型级常量 |
| `log_name` | `constant` | `constant.log_name` | "天擎外部设备告警日志" | `—` | confirmed；expected 事件确认的日志类型级常量 |
...
|---|---|---|---|---|---|
| `roles.target.host.name + source_host` | `gap` | `data_gap.raw_log.client_info.name` | `data_gap.raw_log.client_info.name` | `—` | missing；规则未抽；WPL name 表示 USB 设备名 |
| `roles.related[].device.id` | `gap` | `data_gap.raw_log.device_instance_path` | `data_gap.raw_log.device_instance_path` | `—` | missing；规则未抽 |
| `source_finding.rule.label` | `gap` | `data_gap.raw_log.rule` | `data_gap.raw_log.rule` | `—` | missing；规则未抽 |
| `source_original_event_id` | `projection` | `guid` | "{4D36E967-E325-11CE-BFC1-08002BE10318}" | `—` | confirmed；去花括号 |
| `roles.related[].device.name` | `projection` | `name` | "MULTIFLASHREADERUSBDEVICE" | `—` | confirmed；外设名 |
| `extensions.source_private.vid` | `projection` | `vid` | "12738" | `—` | confirmed；厂商 ID |
| `extensions.source_private.pid` | `projection` | `pid` | "2134" | `—` | confirmed；产品 ID |
| `extensions.source_private.result` | `projection` | `result` | 1 | `—` | confirmed；PDF 放行；同时进 outcome=allowed |
| `roles.target.host.ip` | `projection` | `report_ip` | "198.51.100.63" | `—` | confirmed；受管终端上报 IP |
| `roles.target.host.mac` | `projection` | `mac` | "00-0C-29-14-90-32" | `—` | confirmed；受管终端 MAC |
