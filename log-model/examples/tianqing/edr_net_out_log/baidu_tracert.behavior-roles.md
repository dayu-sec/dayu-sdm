...
句式：主机 `DESKTOP-OPCF5JG` 外联 `198.51.100.207`（baidu.com）；无进程字段。天擎只记录，无检测断言。
...
| object | `endpoint` `198.51.100.207` | `net_out_ip`。`outreach_address=baidu.com` 不升第二客体 domain |
...
- `subject.host.ip` 用 WPL `report_ip=192.0.2.45`。raw `client_ip=198.51.100.215` 未进 WPL，不写。
- `tracert` 未进 WPL，不落信封。
- `is_down` 未进 WPL，不从 raw 补。
- 资产库存字段（内存/IE/语言等）不进本事件。
- MAC：`00-00-5E-00-53-9D` → `00:00:5E:00:53:4E`。
