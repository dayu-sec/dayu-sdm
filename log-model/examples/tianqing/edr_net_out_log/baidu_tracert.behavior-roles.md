...
句式：主机 `DESKTOP-EX08` 外联 `36.112.17.98`（baidu.com）；无进程字段。天擎只记录，无检测断言。

矩阵：网络访问 → 连接发起端 / `network / connect` / 服务、主机或端点。

| 角色 | 判定 | 证据 |
|---|---|---|
| subject | `host` `DESKTOP-EX08` | 无进程。矩阵「连接发起端」；本条不能造 process。旧 expected 已是 host |
...
- `subject.host.ip` 用 WPL `report_ip=192.0.2.184`。raw `client_ip=192.0.2.189` 未进 WPL，不写。
- `tracert` 未进 WPL，不落信封。
- `is_down` 未进 WPL，不从 raw 补。
- 资产库存字段（内存/IE/语言等）不进本事件。
- MAC：`00-00-5E-00-53-AC` → `00:00:5e:00:53:ac`。
