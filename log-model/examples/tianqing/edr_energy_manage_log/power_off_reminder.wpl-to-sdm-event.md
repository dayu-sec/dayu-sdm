...
> 样例：`power_off_reminder.expected-sdm-event.json`；WPL 字段 35 个。
...
| 来源 | SDM 落位 | 值 | 说明 |
|---|---|---|---|
| `data_gap.raw_log.content` | `source_finding.title` | "能耗告警：终端超过 264 小时未关机" | 原始日志存在但 WPL 未抽 |
...
| `event_type` | `constant` | `constant` | "generic_event" | `—` | confirmed；06 无能耗管理类型 |
| `event_category` | `constant` | `constant` | "alert" | `—` | confirmed；能耗策略告警 |
...
| `mapping_id` | `constant` | `constant.mapping_id` | "qax.tianqing.edr_energy_manage_log" | `—` | confirmed；expected 事件确认的日志类型级常量 |
| `data_src_product` | `constant` | `constant.data_src_product` | "tianqing" | `—` | confirmed；expected 事件确认的日志类型级常量 |
| `log_type` | `constant` | `constant.log_type` | "edr_energy_manage_log" | `—` | confirmed；expected 事件确认的日志类型级常量 |
| `data_src_category` | `constant` | `constant.data_src_category` | "endpoint_security" | `—` | confirmed；expected 事件确认的日志类型级常量 |
| `log_name` | `constant` | `constant.log_name` | "天擎能耗管理日志" | `—` | confirmed；expected 事件确认的日志类型级常量 |
...
|---|---|---|---|---|---|
| `source_finding.title` | `gap` | `data_gap.raw_log.content` | `data_gap.raw_log.content` | `—` | missing；规则未抽 |
