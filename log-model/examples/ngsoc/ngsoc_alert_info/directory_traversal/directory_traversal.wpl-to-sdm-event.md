...
|---|---|---|---|---|---|
| `event_id` | `derived` | `derived.sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` | `'evt-' + sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` -> "evt-3cba9002fe693df81a660874538e4111e999f65da7ee4d2980cc34cd653df6e9" | `—` | confirmed；来源序列驱动的确定性事件 ID |
| `record_kind` | `conditional` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历"` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历" then chars(finding) else no_match` -> "finding" | `no_match` | confirmed；NGSOC 来源告警 |
| `event_domain` | `conditional` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历"` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历" then chars(threat) else no_match` -> "threat" | `no_match` | confirmed；来源目录遍历检测声明 |
| `event_type` | `conditional` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历" && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志"` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历" && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志" then chars(network_http) else no_match` -> "network_http" | `no_match` | confirmed；relevantLogsType 明确为天堤网页漏洞利用日志，支持网络 HTTP 事件类型 |
| `operation` | `conditional` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历" && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志"` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历" && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志" then chars() else no_match` -> "" | `no_match` | confirmed；当前 HTTP 告警分支无方法或标准动作证据；不得影响同 mapping_id 的其他子类型 |
...
|---|---|---|---|---|---|
| `occur_time` | `transform` | `timestamp` | `parse_time_ms(wpl.timestamp,'Asia/Shanghai')` -> 1734486892000 | `drop_and_review` | confirmed；WPL 输出仍为 chars；SDM 装配转换为毫秒 |
| `source_original_event_id` | `projection` | `seq` | "12934844" | `—` | confirmed；来源告警序列热字段 |
| `source_finding.original_id` | `projection` | `seq` | "12934844" | `—` | confirmed；来源 finding 原始 ID |
| `roles.source.endpoint.ip` | `transform` | `srcIp` | `exactly_one(parse_ip_list(wpl.srcIp))` -> "192.0.2.26" | `drop_and_review` | confirmed；单一通信源 IP |
| `source_ip` | `projection` | `roles.source.endpoint.ip` | "192.0.2.26" | `—` | confirmed；从源端点投影热字段 |
| `roles.target.endpoint.ip` | `transform` | `dstIp` | `exactly_one(parse_ip_list(wpl.dstIp))` -> "198.51.100.139" | `drop_and_review` | confirmed；单一通信目标 IP |
| `target_ip` | `projection` | `roles.target.endpoint.ip` | "198.51.100.139" | `—` | confirmed；从目标端点投影热字段 |
| `roles.target.endpoint.port` | `transform` | `dport` | `exactly_one(parse_port_list(wpl.dport))` -> 17001 | `drop_and_review` | confirmed；单一目标端口 |
| `target_port` | `projection` | `roles.target.endpoint.port` | 17001 | `—` | confirmed；从目标端点投影热字段 |
| `source_finding.victim.endpoint.port` | `transform` | `dport` | `exactly_one(parse_port_list(wpl.dport))` -> 17001 | `omit_and_review` | confirmed；来源受害方端口声明 |
| `extensions.source_private.source_ports` | `transform` | `sport` | `parse_port_list(wpl.sport)` -> [60266, 61329] | `drop_invalid_items_and_review` | confirmed；保留聚合源端口；不生成 source_port |
| `extensions.source_private.request_targets` | `transform` | `uri` | `parse_uri_list(wpl.uri,entry_prefix='/')` -> ["/hnnx/poserver.zz"] | `preserve_raw_and_review` | confirmed；按条目前导斜杠识别边界；不拼造完整 URL |
| `roles.related[].domain.name` | `transform` | `domain` | `parse_domain_list(wpl.domain)` -> ["auditapp.hnrcc.bank:17001"] | `drop_invalid_items_and_review` | confirmed；关联域名不声明与目标 IP 的 DNS 关系 |
...
| `source_finding.title` | `projection` | `name` | "目录遍历攻击(通用)" | `—` | confirmed；来源告警标题 |
...
| `source_finding.count` | `transform` | `times` | `to_int(wpl.times)` -> 2 | `omit_and_review` | confirmed；聚合命中次数 |
| `source_finding.confidence` | `projection` | `confidence` | "中" | `—` | confirmed；来源置信度 |
| `source_finding.category.original.name` | `projection` | `ruleCategoryName` | "目录遍历" | `—` | confirmed；来源分类 |
| `source_finding.attacker.endpoint.ip` | `transform` | `attackerContent` | `exactly_one(parse_ip_list(wpl.attackerContent))` -> "192.0.2.26" | `omit_and_review` | confirmed；来源攻击方声明，与通信角色分开 |
| `source_finding.victim.endpoint.ip` | `transform` | `victimContent` | `exactly_one(parse_ip_list(wpl.victimContent))` -> "198.51.100.139" | `omit_and_review` | confirmed；来源受害方声明，与通信角色分开 |
| `source_finding.victim.geo` | `transform` | `dipGeo` | `geo_by_ip(parse_json(wpl.dipGeo),exactly_one(parse_ip_list(wpl.victimContent)))` -> {"continent": {"name": "亚洲"}, "country": {"name": "中国"}, "region": {"name": "湖南"}, "city": {"name": "长沙"}} | `omit_and_review` | confirmed；目标 Geo 按受害方 IP 取值；0.0/0.0 哨兵坐标省略 |
| `source_finding.rule.id` | `transform` | `ruleId` | `string(exactly_one(parse_integer_list(wpl.ruleId)))` -> "5520" | `omit_and_review` | confirmed；来源规则 ID |
| `source_finding.rule.label` | `transform` | `ruleName` | `exactly_one(parse_text_list(wpl.ruleName))` -> "目录遍历攻击(通用)" | `omit_and_review` | confirmed；与 ruleId 配对的来源检测规则名称 |
| `extensions.source_private.relevant_rule_name` | `projection` | `relevantRuleName` | "升级-预置-网络探针检测到目录遍历事件" | `—` | confirmed；触发告警的关联规则名称；不与检测规则 ID 配对 |
| `source_finding.attention.content` | `projection` | `attentionValue` | "URI资源 : /hnnx/poserver.zz" | `—` | confirmed；来源关注内容 |
| `source_finding.behavior` | `conditional` | `name/ruleCategoryName` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历" then chars(directory_traversal) else omit` -> "directory_traversal" | `omit` | confirmed；来源目录遍历分类声明；URI 仅保留原始探测目标，不推断具体 ../ 载荷 |
| `extensions.source_private.directory_traversal_variant` | `conditional` | `name/ruleCategoryName` | `when wpl.name == "目录遍历攻击(通用)" && wpl.ruleCategoryName == "目录遍历" then chars(generic) else omit` -> "generic" | `omit` | confirmed；日志标题明确为通用目录遍历，未提供更细变体证据 |
...
| `extensions.unmapped.att_ck_original` | `projection` | `attCk` | "[利用面向公众的应用程序]" | `—` | confirmed；来源文本不是 ATT&CK 技术编号 |
| `extensions.source_private.wpl_update_time` | `projection` | `update_time` | "2026-12-18 16:02:00" | `—` | confirmed；WPL 从无年份 Syslog 头推断，仅供审计 |
| `extensions.source_private.latest_timestamp` | `projection` | `latestTimestamp` | 1734508582000 | `—` | confirmed；WPL 已转毫秒 |
