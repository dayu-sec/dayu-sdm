...
| `roles.related[].ref_id` | `derived` | `derived.entity_ref(domain,parse_domain_list(wpl.domain)[])` | `map(entity_ref(domain,item),parse_domain_list(wpl.domain))` -> ["domain::sdm.example.internal", "domain::sdm.example.internal"] | `drop_invalid_items_and_review` | confirmed；每个域名建立稳定关联引用 |
...
| `extensions.source_private.request_targets` | `transform` | `uri` | `parse_uri_list(wpl.uri,entry_prefix='/')` -> ["/login?url=http://sdm.example.internal/'/**/and(select'1'from/**/pg_sleep(5))::text>'0", "/login?url=http://sdm.example.internal/\"/**/and(select\"1\"from/**/pg_sleep(5))::text>\"0", "/login?url=http://sdm.example.internal//**/and(select/**/1/**/from/**/pg_sleep(5))>0/**/", "/api/strategys?pageSize=10&pageNumber=1&type=2&query=123''/**/and(select'1'from/**/pg_sleep(5))::text>'0", "/api/strategys?pageSize=10&pageNumber=1&type=2&query=123'/**/and(select/**/1/**/from/**/pg_sleep(5))>0/**/", "/api/strategys/ME454286696117313536/components?pageSize=10&pageNumber=1&type=2&query=123'/**/and(select'1'from/**/pg_sleep(5))::text>'0", "/api/strategys/ME454286696117313536/components?pageSize=10&pageNumber=1&type=2&query=123\"/**/and(select\"1\"from/**/pg_sleep(5))::text>\"0", "/api/strategys/ME462186493725777920/managers?pageSize=10&pageNumber=1/**/and(select/**/1/**/from/**/pg_sleep(5))>0/**/"] | `preserve_raw_and_review` | confirmed；按条目前导斜杠识别边界；不拼造完整 URL |
| `roles.related[].domain.name` | `transform` | `domain` | `parse_domain_list(wpl.domain)` -> ["sdm.example.internal", "sdm.example.internal"] | `drop_invalid_items_and_review` | confirmed；关联域名不声明与目标 IP 的 DNS 关系 |
