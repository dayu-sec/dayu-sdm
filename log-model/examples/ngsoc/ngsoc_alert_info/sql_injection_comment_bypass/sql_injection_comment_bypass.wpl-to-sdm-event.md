...
> 样例：`sql_injection_comment_bypass.expected-sdm-event.json`。机器可读 JSON 记录全部 `67` 个 WPL 字段的处理结论。
...
|---|---|---|---|---|---|
| `event_id` | `derived` | `derived.sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` | `'evt-' + sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` -> "evt-08f3dc269d6e4e86f64eed1f7c4ccb33b8dc32b27688142b382e50a3e4792c93" | `—` | confirmed；来源序列驱动的确定性事件 ID |
| `record_kind` | `conditional` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入"` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入" then chars(finding) else no_match` -> "finding" | `no_match` | confirmed；NGSOC 来源告警 |
| `event_domain` | `conditional` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入"` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入" then chars(threat) else no_match` -> "threat" | `no_match` | confirmed；来源 SQL 注入检测声明 |
| `event_type` | `conditional` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入" && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志"` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入" && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志" then chars(network_http) else no_match` -> "network_http" | `no_match` | confirmed；relevantLogsType 明确为天堤网页漏洞利用日志，支持网络 HTTP 事件类型 |
| `operation` | `conditional` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入" && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志"` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入" && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志" then chars() else no_match` -> "" | `no_match` | confirmed；当前 HTTP 告警分支无方法或标准动作证据；不得影响同 mapping_id 的其他子类型 |
...
| `extensions.source_private.syslog_header_time` | `derived` | `raw_syslog.header.time` | `read(raw_syslog.header.time)` -> "Dec 18 16:01:09" | `—` | confirmed；不含年份的 Syslog 头时间审计值 |
...
|---|---|---|---|---|---|
| `occur_time` | `transform` | `timestamp` | `parse_time_ms(wpl.timestamp,'Asia/Shanghai')` -> 1734506889000 | `drop_and_review` | confirmed；WPL 输出仍为 chars；SDM 装配转换为毫秒 |
| `source_original_event_id` | `projection` | `seq` | "12936309" | `—` | confirmed；来源告警序列热字段 |
| `source_finding.original_id` | `projection` | `seq` | "12936309" | `—` | confirmed；来源 finding 原始 ID |
...
| `extensions.source_private.source_ports` | `transform` | `sport` | `parse_port_list(wpl.sport)` -> [51546, 51560, 51534, 33236, 33220, 38310, 38334, 44610] | `drop_invalid_items_and_review` | confirmed；保留聚合源端口；不生成 source_port |
| `extensions.source_private.request_targets` | `transform` | `uri` | `parse_uri_list(wpl.uri,entry_prefix='/')` -> ["/login?url=http://host-5c568da6.sdm.example.internal/'/**/and(select'1'from/**/pg_sleep(5))::text>'0", "/login?url=http://host-5c568da6.sdm.example.internal/\"/**/and(select\"1\"from/**/pg_sleep(5))::text>\"0", "/login?url=http://host-5c568da6.sdm.example.internal//**/and(select/**/1/**/from/**/pg_sleep(5))>0/**/", "/api/strategys?pageSize=10&pageNumber=1&type=2&query=123''/**/and(select'1'from/**/pg_sleep(5))::text>'0", "/api/strategys?pageSize=10&pageNumber=1&type=2&query=123'/**/and(select/**/1/**/from/**/pg_sleep(5))>0/**/", "/api/strategys/ME454286696117313536/components?pageSize=10&pageNumber=1&type=2&query=123'/**/and(select'1'from/**/pg_sleep(5))::text>'0", "/api/strategys/ME454286696117313536/components?pageSize=10&pageNumber=1&type=2&query=123\"/**/and(select\"1\"from/**/pg_sleep(5))::text>\"0", "/api/strategys/ME462186493725777920/managers?pageSize=10&pageNumber=1/**/and(select/**/1/**/from/**/pg_sleep(5))>0/**/"] | `preserve_raw_and_review` | confirmed；按条目前导斜杠识别边界；不拼造完整 URL |
...
| `source_finding.title` | `projection` | `name` | "SQL注入攻击_注释字符绕过" | `—` | confirmed；来源告警标题 |
...
| `source_finding.count` | `transform` | `times` | `to_int(wpl.times)` -> 8 | `omit_and_review` | confirmed；聚合命中次数 |
...
| `source_finding.rule.id` | `transform` | `ruleId` | `string(exactly_one(parse_integer_list(wpl.ruleId)))` -> "304481" | `omit_and_review` | confirmed；来源规则 ID |
| `source_finding.rule.label` | `transform` | `ruleName` | `exactly_one(parse_text_list(wpl.ruleName))` -> "SQL注入攻击_注释字符绕过" | `omit_and_review` | confirmed；与 ruleId 配对的来源检测规则名称 |
...
| `source_finding.behavior` | `conditional` | `name/ruleCategoryName/uri` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入" && any_all_contains(parse_uri_list(wpl.uri, entry_prefix="/"), ["/**/", "pg_sleep(5)"], case_insensitive=true) then chars(sql_injection) else omit` -> "sql_injection" | `omit` | confirmed；SQL 注入来源声明及注释分隔的 pg_sleep 载荷 |
| `extensions.source_private.sql_injection_variant` | `conditional` | `name/ruleCategoryName/uri` | `when wpl.name == "SQL注入攻击_注释字符绕过" && wpl.ruleCategoryName == "SQL注入" && any_all_contains(parse_uri_list(wpl.uri, entry_prefix="/"), ["/**/", "pg_sleep(5)"], case_insensitive=true) then chars(comment_bypass_pg_sleep) else omit` -> "comment_bypass_pg_sleep" | `omit` | confirmed；名称与任一 URI 中的 /**/ 和 pg_sleep(5) 共同确认注释绕过变体 |
...
| `extensions.source_private.wpl_update_time` | `projection` | `update_time` | "2026-12-18 16:01:09" | `—` | confirmed；WPL 从无年份 Syslog 头推断，仅供审计 |
| `extensions.source_private.latest_timestamp` | `projection` | `latestTimestamp` | 1734508608000 | `—` | confirmed；WPL 已转毫秒 |
...
| `extensions.source_private.relevant_asset_names` | `transform` | `relevantAssetsName` | `parse_text_list(wpl.relevantAssetsName)` -> ["示例集团信贷系统"] | `preserve_raw_and_review` | confirmed；来源关联资产名称 |
| `extensions.source_private.relevant_asset_groups` | `transform` | `relevantAssetsGroup` | `parse_text_list(wpl.relevantAssetsGroup)` -> ["互联网业务区", "已删除", "示例集团"] | `preserve_raw_and_review` | confirmed；来源关联资产组 |
| `extensions.source_private.relevant_network_segments` | `transform` | `relevantNetworkSegmentId` | `parse_text_list(wpl.relevantNetworkSegmentId)` -> ["示例集团办公网", "互联网业务区", "运维管理区"] | `preserve_raw_and_review` | confirmed；来源关联网络分区 |
