...
> 样例：`sql_injection_sleep_function.expected-sdm-event.json`。机器可读 JSON 记录全部 `67` 个 WPL 字段的处理结论。
...
|---|---|---|---|---|---|
| `event_id` | `derived` | `derived.sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` | `'evt-' + sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` -> "evt-47d5b0d8036453990c43912cf64f5ecca539c800739b908fc634a78bd6be2628" | `—` | confirmed；来源序列驱动的确定性事件 ID |
| `record_kind` | `conditional` | `when wpl.name == "SQL注入攻击_SLEEP休眠函数注入" && wpl.ruleCategoryName == "SQL注入" && any_contains(parse_uri_list(wpl.uri, entry_prefix="/"), "sleep(5)")` | `when wpl.name == "SQL注入攻击_SLEEP休眠函数注入" && wpl.ruleCategoryName == "SQL注入" && any_contains(parse_uri_list(wpl.uri, entry_prefix="/"), "sleep(5)") then chars(finding) else no_match` -> "finding" | `no_match` | confirmed；NGSOC 来源告警 |
| `event_domain` | `conditional` | `when wpl.name == "SQL注入攻击_SLEEP休眠函数注入" && wpl.ruleCategoryName == "SQL注入" && any_contains(parse_uri_list(wpl.uri, entry_prefix="/"), "sleep(5)")` | `when wpl.name == "SQL注入攻击_SLEEP休眠函数注入" && wpl.ruleCategoryName == "SQL注入" && any_contains(parse_uri_list(wpl.uri, entry_prefix="/"), "sleep(5)") then chars(threat) else no_match` -> "threat" | `no_match` | confirmed；来源 SQL 注入检测声明 |
| `event_type` | `conditional` | `when wpl.name == "SQL注入攻击_SLEEP休眠函数注入" && wpl.ruleCategoryName == "SQL注入" && any_contains(parse_uri_list(wpl.uri, entry_prefix="/"), "sleep(5)") && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志"` | `when wpl.name == "SQL注入攻击_SLEEP休眠函数注入" && wpl.ruleCategoryName == "SQL注入" && any_contains(parse_uri_list(wpl.uri, entry_prefix="/"), "sleep(5)") && exactly_one(parse_text_list(wpl.relevantLogsType)) == "天堤网页漏洞利用日志" then chars(network_http) else no_match` -> "network_http" | `no_match` | confirmed；网页漏洞利用日志与 URI 载荷共同支持 HTTP 请求上下文 |
| `operation` | `constant` | `constant.empty_operation_for_network_http` | "" | `—` | confirmed；无 HTTP 方法或标准动作证据 |
...
| `extensions.source_private.syslog_header_time` | `derived` | `raw_syslog.header.time` | `read(raw_syslog.header.time)` -> "Dec 18 16:01:00" | `—` | confirmed；不含年份的 Syslog 头时间审计值 |
...
|---|---|---|---|---|---|
| `occur_time` | `transform` | `timestamp` | `parse_time_ms(wpl.timestamp,'Asia/Shanghai')` -> 1734506882000 | `drop_and_review` | confirmed；WPL 输出仍为 chars；SDM 装配转换为毫秒 |
| `source_original_event_id` | `projection` | `seq` | "12936302" | `—` | confirmed；来源告警序列热字段 |
| `source_finding.original_id` | `projection` | `seq` | "12936302" | `—` | confirmed；来源 finding 原始 ID |
...
| `extensions.source_private.source_ports` | `transform` | `sport` | `parse_port_list(wpl.sport)` -> [51480, 51468, 51454, 57708, 57696, 57714, 57716, 33136, 33152, 33124] | `drop_invalid_items_and_review` | confirmed；保留聚合源端口；不生成 source_port |
| `extensions.source_private.request_targets` | `transform` | `uri` | `parse_uri_list(wpl.uri,entry_prefix='/')` -> ["/login?url=http://host-5c568da6.sdm.example.internal/\"and sleep(5) and \"1", "/login?url=http://host-5c568da6.sdm.example.internal/'and sleep(5) and '1", "/login?url=http://host-5c568da6.sdm.example.internal/ and sleep(5)", "/apis/login", "/api/strategys?pageSize=10&pageNumber=1&type=2&query=123''and sleep(5) and '1", "/api/strategys?pageSize=10&pageNumber=1&type=2&query=123'\"and sleep(5) and \"1", "/api/strategys?pageSize=10&pageNumber=1&type=2&query=123' and sleep(5)", "/api/strategys/ME454286696117313536/components?pageSize=10&pageNumber=1&type=2&query=123'and sleep(5) and '1", "/api/strategys/ME454286696117313536/components?pageSize=10&pageNumber=1&type=2&query=123\"and sleep(5) and \"1", "/api/strategys/ME454286696117313536/components?pageSize=10&pageNumber=1&type=2&query=123 and sleep(5)"] | `preserve_raw_and_review` | confirmed；按条目前导斜杠识别边界；不拼造完整 URL |
...
| `source_finding.title` | `projection` | `name` | "SQL注入攻击_SLEEP休眠函数注入" | `—` | confirmed；来源告警标题 |
...
| `source_finding.count` | `transform` | `times` | `to_int(wpl.times)` -> 14 | `omit_and_review` | confirmed；聚合命中次数 |
...
| `source_finding.rule.id` | `transform` | `ruleId` | `string(exactly_one(parse_integer_list(wpl.ruleId)))` -> "11724" | `omit_and_review` | confirmed；来源规则 ID |
| `source_finding.rule.label` | `projection` | `relevantRuleName` | "调整-网络探针检测到SQL注入事件" | `—` | confirmed；来源规则名称 |
| `source_finding.attention.content` | `projection` | `attentionValue` | "目的IP : 192.0.2.146" | `—` | confirmed；来源关注内容 |
| `source_finding.behavior` | `conditional` | `name/ruleCategoryName/uri` | `when wpl.name == "SQL注入攻击_SLEEP休眠函数注入" && wpl.ruleCategoryName == "SQL注入" && any_contains(parse_uri_list(wpl.uri, entry_prefix="/"), "sleep(5)") then chars(sql_injection) else omit` -> "sql_injection" | `omit` | confirmed；SQL 注入来源声明及 sleep(5) 载荷 |
| `extensions.source_private.sql_injection_variant` | `conditional` | `name/ruleCategoryName/uri` | `when wpl.name == "SQL注入攻击_SLEEP休眠函数注入" && wpl.ruleCategoryName == "SQL注入" && any_contains(parse_uri_list(wpl.uri, entry_prefix="/"), "sleep(5)") then chars(sleep_function) else omit` -> "sleep_function" | `omit` | confirmed；名称和 URI 子串共同确认 SLEEP 函数变体 |
...
| `extensions.source_private.wpl_update_time` | `projection` | `update_time` | "2026-12-18 16:01:00" | `—` | confirmed；WPL 从无年份 Syslog 头推断，仅供审计 |
| `extensions.source_private.latest_timestamp` | `projection` | `latestTimestamp` | 1734508595000 | `—` | confirmed；WPL 已转毫秒 |
