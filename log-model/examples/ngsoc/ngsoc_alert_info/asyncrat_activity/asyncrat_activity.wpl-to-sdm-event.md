...
> 样例：`asyncrat_activity.expected-sdm-event.behavior.json（现行）；旧 `*.expected-sdm-event.json` 为 55 列对照`。机器可读 JSON 记录全部 `67` 个 WPL 字段的处理结论。
...
|---|---|---|---|---|---|
| `event_id` | `derived` | `derived.sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` | `'evt-' + sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` -> "evt-fba3dc1a4476bfdff0fdfaecb474aa47ad392cadd7a0107e55c606e9c519c179" | `—` | confirmed；来源序列驱动的确定性事件 ID |
| `record_kind` | `conditional` | `when wpl.name == "AsyncRAT远控木马活动事件" && wpl.ruleCategoryName == "远控木马"` | `when wpl.name == "AsyncRAT远控木马活动事件" && wpl.ruleCategoryName == "远控木马" then chars(finding) else no_match` -> "finding" | `no_match` | confirmed；NGSOC 来源告警 |
| `event_domain` | `conditional` | `when wpl.name == "AsyncRAT远控木马活动事件" && wpl.ruleCategoryName == "远控木马"` | `when wpl.name == "AsyncRAT远控木马活动事件" && wpl.ruleCategoryName == "远控木马" then chars(threat) else no_match` -> "threat" | `no_match` | confirmed；来源远控木马检测声明 |
| `event_type` | `conditional` | `when wpl.name == "AsyncRAT远控木马活动事件" && wpl.ruleCategoryName == "远控木马" && exactly_one(parse_text_list(wpl.protocol)) == "DNS" && exactly_one(parse_domain_list(wpl.domain)) == "xred.mooo.com"` | `when wpl.name == "AsyncRAT远控木马活动事件" && wpl.ruleCategoryName == "远控木马" && exactly_one(parse_text_list(wpl.protocol)) == "DNS" && exactly_one(parse_domain_list(wpl.domain)) == "xred.mooo.com" then chars(network_dns) else no_match` -> "network_dns" | `no_match` | confirmed；protocol=DNS 且 domain 提供单一域名 |
| `operation` | `conditional` | `when wpl.name == "AsyncRAT远控木马活动事件" && wpl.ruleCategoryName == "远控木马" && exactly_one(parse_text_list(wpl.protocol)) == "DNS" && exactly_one(parse_domain_list(wpl.domain)) == "xred.mooo.com"` | `when wpl.name == "AsyncRAT远控木马活动事件" && wpl.ruleCategoryName == "远控木马" && exactly_one(parse_text_list(wpl.protocol)) == "DNS" && exactly_one(parse_domain_list(wpl.domain)) == "xred.mooo.com" then chars(query) else no_match` -> "query" | `no_match` | confirmed；DNS 协议与单一 domain 字段共同支持域名查询语义 |
| `roles.source.ref_id` | `derived` | `derived.entity_ref(endpoint,exactly_one(parse_ip_list(wpl.srcIp)))` | `entity_ref(endpoint,exactly_one(parse_ip_list(wpl.srcIp)))` -> "endpoint::192.0.2.38" | `drop_and_review` | confirmed；源端点稳定引用 |
...
| `roles.related[].ref_id` | `derived` | `derived.entity_ref(domain,parse_domain_list(wpl.domain)[])` | `map(entity_ref(domain,item),parse_domain_list(wpl.domain))` -> ["domain::xred.mooo.com"] | `drop_invalid_items_and_review` | confirmed；每个域名建立稳定关联引用 |
...
| `extensions.source_private.syslog_header_time` | `derived` | `raw_syslog.header.time` | `read(raw_syslog.header.time)` -> "Dec 18 16:01:21" | `—` | confirmed；不含年份的 Syslog 头时间审计值 |
...
|---|---|---|---|---|---|
| `occur_time` | `transform` | `timestamp` | `parse_time_ms(wpl.timestamp,'Asia/Shanghai')` -> 1734481327000 | `drop_and_review` | confirmed；WPL 输出仍为 chars；SDM 装配转换为毫秒 |
| `source_original_event_id` | `projection` | `seq` | "12933970" | `—` | confirmed；来源告警序列热字段 |
| `source_finding.original_id` | `projection` | `seq` | "12933970" | `—` | confirmed；来源 finding 原始 ID |
| `roles.source.endpoint.ip` | `transform` | `srcIp` | `exactly_one(parse_ip_list(wpl.srcIp))` -> "192.0.2.38" | `drop_and_review` | confirmed；单一通信源 IP |
| `source_ip` | `projection` | `roles.source.endpoint.ip` | "192.0.2.38" | `—` | confirmed；从源端点投影热字段 |
...
| `roles.related[].domain.name` | `transform` | `domain` | `parse_domain_list(wpl.domain)` -> ["xred.mooo.com"] | `drop_invalid_items_and_review` | confirmed；关联域名不声明与目标 IP 的 DNS 关系 |
...
| `source_finding.title` | `projection` | `name` | "AsyncRAT远控木马活动事件" | `—` | confirmed；来源告警标题 |
...
| `source_finding.count` | `transform` | `times` | `to_int(wpl.times)` -> 20 | `omit_and_review` | confirmed；聚合命中次数 |
...
| `source_finding.victim.endpoint.ip` | `transform` | `victimContent` | `exactly_one(parse_ip_list(wpl.victimContent))` -> "192.0.2.38" | `omit_and_review` | confirmed；来源受害方声明对应通信主体 |
...
| `source_finding.rule.label` | `transform` | `ruleName` | `exactly_one(parse_text_list(wpl.ruleName))` -> "AsyncRAT远控木马活动事件" | `omit_and_review` | confirmed；ruleId 为空，仅保留来源检测规则名称 |
| `source_finding.ioc.type` | `transform` | `iocType` | `exactly_one(parse_text_list(wpl.iocType))` -> "域名" | `omit_and_review` | confirmed；来源 IOC 类型 |
| `source_finding.ioc.value` | `transform` | `ioc` | `exactly_one(parse_text_list(wpl.ioc))` -> "xred.mooo.com" | `omit_and_review` | confirmed；来源 IOC 域名 |
| `source_finding.malware.name` | `transform` | `maliciousFamily` | `exactly_one(parse_text_list(wpl.maliciousFamily))` -> "AsyncRAT" | `omit_and_review` | confirmed；来源恶意家族名称 |
| `facets.dns.question.name` | `transform` | `domain` | `exactly_one(parse_domain_list(wpl.domain))` -> "xred.mooo.com" | `omit_and_review` | confirmed；DNS 查询域名；不推断应答 |
...
| `source_finding.attention.content` | `projection` | `attentionValue` | "源IP : 192.0.2.38" | `—` | confirmed；来源关注内容 |
| `source_finding.behavior` | `conditional` | `name/ruleCategoryName` | `when wpl.name == "AsyncRAT远控木马活动事件" && wpl.ruleCategoryName == "远控木马" then chars(remote_access_trojan) else omit` -> "remote_access_trojan" | `omit` | confirmed；来源名称和分类共同声明远控木马活动；恶意家族另行映射 |
