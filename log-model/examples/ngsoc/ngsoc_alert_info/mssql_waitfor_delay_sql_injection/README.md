# NGSOC MSSQL WAITFOR DELAY SQL 注入告警样例

原始 Syslog KV 来自仓库 `log-model/models/wpl/ngsoc/sample.dat:20`。NGSOC 声明 `192.0.2.238` 对 `192.0.2.146:80` 发起 MSSQL WAITFOR DELAY SQL 注入企图，聚合 `17` 次命中；请求目标中明确出现 `waitfor delay 0:0:5`，但日志没有 HTTP 方法、传输协议、响应、阻断或 SQL 执行结果。
...
- `ruleId=[310189]` 与 `ruleName=[MSSQL Waitfor语句SQL注入攻击]` 配对写入 `source_finding.rule`；`relevantRuleName=调整-网络探针检测到SQL注入事件` 是关联规则名称，独立保存在 `extensions.source_private.relevant_rule_name`。
- `10` 个源端口保存在 `extensions.source_private.source_ports`，`source_port=null`。
- `10` 个相对 URI 保存在 `request_targets`，不拼造单一 URL；两个域名仅作为 `roles.related`，没有证据将其绑定到目标 IP。
...
- Syslog 头缺少年份，`update_time=2026-12-18 16:01:02` 是 WPL 推断值；正文 `timestamp=2024-12-18 15:28:14` 在 WPL 输出中仍为 chars，由 SDM 装配按 Asia/Shanghai 转为毫秒事件时间。
