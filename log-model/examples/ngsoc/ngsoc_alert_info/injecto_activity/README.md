...
- 主体：通信源端点 `203.0.113.143`，也是来源 `victimContent` 声明的受害主机。
...
- `ruleId=[]`，仅将 `ruleName=Injecto远控木马活动事件` 作为来源规则名称；`relevantRuleName` 独立保存。
...
- Syslog 头缺少年份，`update_time=2026-12-18 16:01:31` 是 WPL 推断值；正文 `timestamp=2024-12-18 08:59:57` 在 WPL 输出中仍为 chars，由 SDM 装配按 Asia/Shanghai 转为毫秒事件时间。
