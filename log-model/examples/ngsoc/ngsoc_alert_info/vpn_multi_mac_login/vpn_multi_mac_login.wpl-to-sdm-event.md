...
> 样例：`vpn_multi_mac_login.expected-sdm-event.json`。机器可读 JSON 记录全部 `67` 个 WPL 字段的处理结论。
...
| `extensions.source_private.syslog_priority/syslog_host/syslog_header_time` | `derived` | `raw_syslog.header` | `parse_syslog_header(raw_input.original_payload)` -> {"priority": 14, "host": "ngsoc93.qax.cn", "time": "Dec 18 16:01:50"} | `—` | confirmed；保留 Syslog 头审计值 |
...
| `roles.related[].endpoint` | `transform` | `attackerContent/srcIp` | `pair_alternating_ip_mac(wpl.attackerContent, wpl.srcIp)` -> [{"ip": "203.0.113.8", "mac": "0a:66:35:07:a5:70"}, {"ip": "203.0.113.141", "mac": "4a:7a:ab:43:1c:d0"}] | `drop_and_review` | confirmed；只有严格交替且 IP 顺序与 srcIp 一致时才配对；不解释为攻击方 |
