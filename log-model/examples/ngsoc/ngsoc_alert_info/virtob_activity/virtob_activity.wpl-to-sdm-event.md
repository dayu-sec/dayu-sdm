...
> 样例：`virtob_activity.expected-sdm-event.behavior.json（现行）；旧 `*.expected-sdm-event.json` 为 55 列对照`。机器可读 JSON 记录全部 `67` 个 WPL 字段的处理结论。
...
| `domain` | `roles.related[].domain.name` | 解析域名列表 | DNS/IOC 相关域名；无请求/响应方向证据，不写入 `dns.question` |
...
| `attCk` | 不落库 | 空值不构造 | 当前样例没有 ATT&CK 技术 ID |

## 映射边界

- 不推断 DNS 查询、应答、TCP/UDP、阻断或蠕虫传播成功。
...
|---|---|---|---|---|---|
| `event_id` | `derived` | `derived.sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` | `'evt-' + sha256(tenant_id+'|'+mapping_id+'|'+wpl.seq)` -> "evt-6c0667f612fa9cbdc4a1f783da9db10687262f6e1f5a31c080b611863ef41bf3" | `—` | confirmed；来源序列驱动的确定性事件 ID |
| `record_kind` | `conditional` | `when wpl.name == "Virtob" && wpl.ruleCategoryName == "网络蠕虫"` | `when wpl.name == "Virtob" && wpl.ruleCategoryName == "网络蠕虫" then chars(finding) else no_match` -> "finding" | `no_match` | confirmed；NGSOC 来源告警 |
| `event_domain` | `conditional` | `when wpl.name == "Virtob" && wpl.ruleCategoryName == "网络蠕虫"` | `when wpl.name == "Virtob" && wpl.ruleCategoryName == "网络蠕虫" then chars(threat) else no_match` -> "threat" | `no_match` | confirmed；来源网络蠕虫检测声明 |
| `event_type` | `conditional` | `when wpl.name == "Virtob" && wpl.ruleCategoryName == "网络蠕虫" && exactly_one(parse_text_list(wpl.protocol)) == "DNS" && length(parse_domain_list(wpl.domain)) >= 1` | `when wpl.name == "Virtob" && wpl.ruleCategoryName == "网络蠕虫" && exactly_one(parse_text_list(wpl.protocol)) == "DNS" && length(parse_domain_list(wpl.domain)) >= 1 then chars(network_dns) else no_match` -> "network_dns" | `no_match` | confirmed；protocol=DNS 且 domain 至少提供一个有效域名；不绑定具体 IOC 值 |
| `operation` | `conditional` | `when wpl.name == "Virtob" && wpl.ruleCategoryName == "网络蠕虫" && exactly_one(parse_text_list(wpl.protocol)) == "DNS" && length(parse_domain_list(wpl.domain)) >= 1` | `when wpl.name == "Virtob" && wpl.ruleCategoryName == "网络蠕虫" && exactly_one(parse_text_list(wpl.protocol)) == "DNS" && length(parse_domain_list(wpl.domain)) >= 1 then chars(traffic) else no_match` -> "traffic" | `no_match` | confirmed；来源只证明 DNS 相关通信，未提供请求或响应方向 |
| `roles.source.ref_id` | `derived` | `derived.entity_ref(endpoint,exactly_one(parse_ip_list(wpl.srcIp)))` | `entity_ref(endpoint,exactly_one(parse_ip_list(wpl.srcIp)))` -> "endpoint::203.0.113.159" | `drop_and_review` | confirmed；源端点稳定引用 |
| `roles.target.ref_id` | `derived` | `derived.entity_ref(endpoint,exactly_one(parse_ip_list(wpl.dstIp)))` | `entity_ref(endpoint,exactly_one(parse_ip_list(wpl.dstIp)))` -> "endpoint::192.0.2.179" | `drop_and_review` | confirmed；无目标端口时按目标 IP 建立稳定引用 |
| `roles.related[].ref_id` | `derived` | `derived.entity_ref(domain,parse_domain_list(wpl.domain)[])` | `map(entity_ref(domain,item),parse_domain_list(wpl.domain))` -> ["domain::fei9988.3322.org"] | `drop_invalid_items_and_review` | confirmed；每个域名建立稳定关联引用 |
...
| `extensions.source_private.syslog_header_time` | `derived` | `raw_syslog.header.time` | `read(raw_syslog.header.time)` -> "Dec 18 16:01:53" | `—` | confirmed；不含年份的 Syslog 头时间审计值 |
...
|---|---|---|---|---|---|
| `occur_time` | `transform` | `timestamp` | `parse_time_ms(wpl.timestamp,'Asia/Shanghai')` -> 1734451293000 | `drop_and_review` | confirmed；WPL 输出仍为 chars；SDM 装配转换为毫秒 |
| `source_original_event_id` | `projection` | `seq` | "12932970" | `—` | confirmed；来源告警序列热字段 |
| `source_finding.original_id` | `projection` | `seq` | "12932970" | `—` | confirmed；来源 finding 原始 ID |
| `roles.source.endpoint.ip` | `transform` | `srcIp` | `exactly_one(parse_ip_list(wpl.srcIp))` -> "203.0.113.159" | `drop_and_review` | confirmed；单一通信源 IP |
| `source_ip` | `projection` | `roles.source.endpoint.ip` | "203.0.113.159" | `—` | confirmed；从源端点投影热字段 |
| `roles.target.endpoint.ip` | `transform` | `dstIp` | `exactly_one(parse_ip_list(wpl.dstIp))` -> "192.0.2.179" | `drop_and_review` | confirmed；单一通信目标 IP |
| `target_ip` | `projection` | `roles.target.endpoint.ip` | "192.0.2.179" | `—` | confirmed；从目标端点投影热字段 |
| `roles.related[].domain.name` | `transform` | `domain` | `parse_domain_list(wpl.domain)` -> ["fei9988.3322.org"] | `drop_invalid_items_and_review` | confirmed；关联域名不声明与目标 IP 的 DNS 关系 |
...
| `source_finding.title` | `projection` | `name` | "Virtob" | `—` | confirmed；来源告警标题 |
...
| `source_finding.count` | `transform` | `times` | `to_int(wpl.times)` -> 60 | `omit_and_review` | confirmed；聚合命中次数 |
| `source_finding.confidence` | `projection` | `confidence` | "高" | `—` | confirmed；来源置信度 |
| `source_finding.category.original.name` | `projection` | `ruleCategoryName` | "网络蠕虫" | `—` | confirmed；来源分类 |
| `source_finding.victim.endpoint.ip` | `transform` | `victimContent` | `exactly_one(parse_ip_list(wpl.victimContent))` -> "203.0.113.159" | `omit_and_review` | confirmed；来源受害方声明对应通信主体 |
...
| `source_finding.rule.label` | `transform` | `ruleName` | `exactly_one(parse_text_list(wpl.ruleName))` -> "Virtob" | `omit_and_review` | confirmed；ruleId 为空，仅保留来源检测规则名称 |
| `source_finding.ioc.type` | `transform` | `iocType` | `exactly_one(parse_text_list(wpl.iocType))` -> "域名" | `omit_and_review` | confirmed；来源 IOC 类型 |
| `source_finding.ioc.value` | `transform` | `ioc` | `exactly_one(parse_text_list(wpl.ioc))` -> "fei9988.3322.org" | `omit_and_review` | confirmed；来源 IOC 域名 |
| `source_finding.malware.name` | `transform` | `maliciousFamily` | `exactly_one(parse_text_list(wpl.maliciousFamily))` -> "Virtob" | `omit_and_review` | confirmed；来源恶意家族名称 |
...
| `extensions.source_private.relevant_rule_name` | `projection` | `relevantRuleName` | "升级-预置-网络探针检测到网络蠕虫IOC命中事件" | `—` | confirmed；触发告警的关联规则名称；不与检测规则 ID 配对 |
| `source_finding.attention.content` | `projection` | `attentionValue` | "源IP : 203.0.113.159" | `—` | confirmed；来源关注内容 |
| `source_finding.behavior` | `conditional` | `name/ruleCategoryName` | `when wpl.name == "Virtob" && wpl.ruleCategoryName == "网络蠕虫" then chars(network_worm) else omit` -> "network_worm" | `omit` | confirmed；来源名称和分类共同声明网络蠕虫活动；恶意家族另行映射 |
...
| `extensions.unmapped.comm_direction_original` | `transform` | `commDirection` | `exactly_one(parse_text_list(wpl.commDirection))` -> "内到内" | `omit_and_review` | confirmed；字典仅覆盖当前样例值，保留厂商原文 |
| `extensions.source_private.wpl_update_time` | `projection` | `update_time` | "2026-12-18 16:01:53" | `—` | confirmed；WPL 从无年份 Syslog 头推断，仅供审计 |
| `extensions.source_private.latest_timestamp` | `projection` | `latestTimestamp` | 1734508563000 | `—` | confirmed；WPL 已转毫秒 |
...
| `extensions.source_private.relevant_network_segments` | `transform` | `relevantNetworkSegmentId` | `parse_text_list(wpl.relevantNetworkSegmentId)` -> ["石门农商行业务网", "星沙上线环境业务网段", "运维管理区"] | `preserve_raw_and_review` | confirmed；来源关联网络分区 |
