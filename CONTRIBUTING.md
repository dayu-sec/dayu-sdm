...
- 样例日志必须来自测试环境或彻底脱敏：不得包含真实客户 IP、主机名、账号、域名、邮箱、口令、MAC。
- 使用 RFC 5737 文档地址（`192.0.2.0/24`、`198.51.100.0/24`、`203.0.113.0/24`）和 `example.com`。
...
- 破坏性 schema 变更需在 issue 中先讨论并被接受。
- 告警模型目前是实验性 v0.2，不接受与 ADR 冲突的表结构 PR，除非先补 ADR。

## 流程

1. Fork 并从 `main` 切分支。
2. 提交 PR，说明变更类型（映射/字段/枚举/文档）与证据来源。
3. CI 会跑契约检查、样例校验和残留 PII 扫描。
