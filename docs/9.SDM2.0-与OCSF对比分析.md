...
>
> **研究基线：** OCSF 1.8.0（2026-03-18 发布）与 SDM2.0 行为信封 2.0（07 Schema / `sdm_event_behavior`）及 S4 设计。
>
> **决策建议：** 采用“分层兼容”路线：OCSF 作为对外交换、行业分类与兼容性基线，SDM2.0 保留为平台内部事件调查、实体索引、存储查询与告警运营模型。
>
> **状态说明：** 内部现行模型是行为信封 2.0（subject / object / carriers / observation）。文中“当前已实现 / 草案 / 建议”只约束改造项，不把旧五层 `sdm_event` 当写入目标。
...
SDM2.0 当前定位为自研 SOC 的统一安全观测数据模型，以行为事件为事实单元，用 **subject → carriers[] → object** 表达主行为链，用 **observation.observer** 承载观察与判断。在此之上，SDM2.0 还研究或实现了事件宽表、语义分层、S4 实体倒排索引、调查视图和独立告警运营模型。[I1][I2][I3]
...
| 事件角色和行为链 | 提供类专用对象 | 行为句式（subject / carriers / object）主责 |
| 物理存储与查询 | 不规定 | S2/S3/S4 主责 |
| 实体归并和时间线 | 提供对象字段 | SDM 实体 ID 和 S4 索引主责 |
| Finding 交换 | Detection/Vulnerability/Incident Finding | `observation.assertion` 对齐并保留来源语义 |
...
| 分类 | category_uid + class_uid + activity_id + type_uid | behavior.layer + behavior.type + operation + outcome |
| 对象语义 | 按 Class 使用 Actor、User、Process、Endpoint、Device 等 | 按 subject / object / carriers / observation.observer 组织实体对象 |
| 字段约束 | Required/Recommended/Optional/Conditional/Profile | 07 Schema 闭合枚举；typed object 由 object-fields 治理 |
| 告警/发现 | Findings Category，覆盖多类 Finding | 事件内 observation.assertion + 独立平台 sdm_alert |
...
SDM2.0 的行为句式先回答“谁通过什么对谁做了什么”，再在角色中放置 User、Host、Process、File、IP、Domain 等对象，更适合统一时间线、实体跳转和行为链。这是 SDM2.0 不应被 OCSF 直接取代的核心原因。

行为句式同样存在主观性：父进程是 subject 还是 carrier，附件是 carrier 还是 object，必须由对象注册表给出机器可校验的唯一规则，不能只靠文档约定。禁止通用 related 兜底。
...
SDM2.0 当前概念模型将来源产品判断保存在 `observation.assertion` 中，另外为平台统一 Alert 设计了六类对象：
...
SDM2.0 S4 将 `sdm_event_behavior` 作为事件事实源，将物理派生表 `sdm_entity_event_index_mv` 作为实体 posting 索引，并定义实体 ID 版本、角色序号、主对端、水位、重建和查询回退。这属于存储服务契约，不属于 OCSF 职责，应继续保留。[I3]
...
| 外部告警/Finding 归一 | 强 | 中 | 以 OCSF 完善 observation.assertion 交换投影 |
...
1. 对外生态弱，外部系统不会天然理解行为句式与内部枚举；
2. 分类覆盖、字段必要性和 Profile 治理不及 OCSF；
3. carrier 有价值但映射主观，需要机器可读契约；
4. 旧 `event_category=alert` 已拆：记录性质进 `meta.source_record.record_kind`，判断进 `observation.assertion`；[I2][I5]
...
- 保留 subject、carriers[]、object 与 observation.observer；禁止通用 related；
...
2.0 已落地：来源判断写入 `observation.assertion`，不再使用 Event 内 `source_alert` / 独立 `source_finding` 层。下列 JSON 只作 OCSF Finding 交换投影草案：
...
mapping_status 至少包含 FULL、PARTIAL、UNMAPPED、INVALID。OCSF 字段用于交换、规则迁移和质量治理，不替代 SDM behavior.type 与行为句式查询。
...
| 攻击者/行为者 | Evidence actor/src_endpoint 或 Resource Actor | `attacker_*`、`sip*` | `sdm_alert_entity` 草案；事件 subject/object | 部分；不得把“来源报告的攻击者”冒充已观测 subject |
